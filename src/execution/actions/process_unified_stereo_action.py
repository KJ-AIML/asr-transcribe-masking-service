...
from src.models.transcription_model_adapter import TranscriptionModelAdapter, WhisperAdapter, TyphoonAdapter
...
def _mp_worker_transcribe_channel(
    ...
    try:
        ...
        print(f"[MP Worker {device}] Transcription completed for {channel_label} in {time.time() - channel_start:.2f}s")
...
def _mp_transcribe_single(
    ...
    try:
        print(f"[MP Worker] Transcribing single file for {channel_label} at {time.time():.2f}...")
...
def _mp_transcribe_chunked(
    ...
    print(f"[MP Worker {device}] Chunked transcription for {channel_label} at {chunked_start:.2f}")
    ...
    print(f"[MP Worker {device}] VAD segmentation completed in {time.time() - chunked_start:.2f}s, found {len(segment_info)} segments")
    ...
    for i, chunk in enumerate(segments):
        ...
        print(f"[MP Worker {device}] Processing chunk {i+1}/{len(segments)} at {chunk_start:.2f}...")
        ...
        try:
            chunk_result = asyncio.run(adapter.transcribe_with_model(
            ...
            print(f"[MP Worker {device}] Chunk {i+1} completed in {time.time() - chunk_start:.2f}s, {len(words)} words")
...
class ProcessUnifiedStereoAction:
        ...
        try:
            ...
            if torch.cuda.is_available():
                ...
                if self.device_count >= 2:
                    ...
                    logger.info(f"Using cuda:0 for Agent, cuda:1 for Caller")
                elif self.device_count == 1:
                    self.agent_device = "cuda:0"
                    self.caller_device = "cuda:0"
                    logger.warning("Only 1 GPU available, both channels will use cuda:0 (sequential)")
                else:
    ...
    async def _process_stereo_with_speaker_separation(self, audio_path: str, model_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"Starting multiprocessing transcription...")
            logger.info(f"Agent device: {self.agent_device}, Caller device: {self.caller_device}")
