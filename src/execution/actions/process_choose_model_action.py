...
class ProcessChooseModelAction:
    ...
    async def execute(self, state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"Processing model selection from {len(compare_results)} chunks"
            )

            # Build workflow if needed
            if self._workflow is None:
                logger.debug(
                    "Building choose model to transcribe workflow for first time"
                )
            ...
            logger.info(
                f"Model selection completed: chosen_model={chosen_model}, confidence={confidence}"
            )
