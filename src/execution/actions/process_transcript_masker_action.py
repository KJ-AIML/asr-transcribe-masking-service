...
class ProcessTranscriptMaskerAction:
    ...
    async def execute(self, masker_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            chunk_detections_map = self._map_detections_to_chunks(
                pass_detections, chunked_result["chunks"]
            )
            ...
            async def process_chunk_with_masker(chunk):
                async with semaphore:
                    ...
                    try:
                        ...
                        logger.debug(
                            f"Processing chunk {chunk_id} with {len(detections)} detections"
                        )
                        ...
                        if masker_results_list and len(masker_results_list) > 0:
                            ...
                            if "transcript" in first_result:
                                masked_text = first_result.get(
                                    "transcript", chunk.get("text", "")
                                )
                                logger.debug(
                                    f"Found masked transcript in masker_results for chunk {chunk_id}"
                                )
                                logger.debug(
                                    f"Original text length: {len(chunk.get('text', ''))}, Masked text length: {len(masked_text)}"
                                )
                            else:
                                logger.warning(
                                    f"No transcript found in masker_results for chunk {chunk_id}"
                                )
                                logger.debug(
                                    f"Available keys in masker result: {list(first_result.keys())}"
                                )
                        else:
                            logger.warning(
                                f"No masker_results found for chunk {chunk_id}"
                            )

                        # Debug logging to verify masking
                        if masked_text == chunk.get("text", "") and len(detections) > 0:
                            logger.warning(
                                f"Chunk {chunk_id} has {len(detections)} detections but text was not masked!"
                            )
                        elif len(detections) > 0:
                            logger.debug(
                                f"Chunk {chunk_id} successfully masked {len(detections)} detections"
                            )
                        ...
                        logger.error(
                            f"Masker workflow failed for chunk {chunk_id}: {e}"
                        )
            ...
            # Process all chunks
            tasks = [
                process_chunk_with_masker(chunk) for chunk in chunked_result["chunks"]
            ]
            if tasks:
                masked_chunks = await asyncio.gather(*tasks)
                total_detections_masked = sum(
                    chunk.get("detections_masked", 0) for chunk in masked_chunks
                )

            # Reconstruct full transcript from masked chunks
            masked_transcript = self._reconstruct_transcript(
                masked_chunks, chunked_result["chunks"]
            )

            # Create summary
            processed_chunks = sum(
                1 for chunk in masked_chunks if chunk.get("status") == "success"
            )
            skipped_chunks = sum(
                1
                for chunk in masked_chunks
                if chunk.get("status") == "skipped_no_detections"
            )
            failed_chunks = sum(
                1 for chunk in masked_chunks if chunk.get("status") == "failed"
            )
            ...
            logger.info(
                f"Masker processing completed: {total_detections_masked} detections masked across {processed_chunks} chunks"
            )
    ...
    def _extract_pass_detections(
        self, re_verify_results: List[Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
    ...
    def _map_detections_to_chunks(
        self, detections: List[Dict[str, Any]], chunks: List[Dict[str, Any]]
    ...
    def _reconstruct_transcript(
        self, masked_chunks: List[Dict[str, Any]], original_chunks: List[Dict[str, Any]]
    ) -> str:
        ...
        for original_chunk in original_chunks:
            ...
            if masked_chunk and masked_chunk.get("status") == "success":
                ...
                chunk_text = masked_chunk.get(
                    "masked_text", original_chunk.get("text", "")
                )
            else:
