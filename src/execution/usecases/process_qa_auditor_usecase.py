...
class ProcessQAAuditorUseCase:
    ...
    async def execute(self, process_output: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if original_transcript:
            logger.info(
                f"Found original_transcript at top level, length: {len(original_transcript)}"
            )

        if not original_transcript:
            logger.info(
                f"Not found original_transcript at top level, length: {len(original_transcript)}"
            )

        # Extract original transcript from the masker_result
        masker_result = process_output.get("masker_result", {})
        logger.info(
            f"Masker result keys: {list(masker_result.keys()) if masker_result else 'None'}"
        )

        # Chunk the original transcript into 100-second chunks
        chunked_result = chunk_original_transcript(
            original_transcript, chunk_duration=100, overlap_duration=10
        )
        ...
        async def process_chunk_for_qa(chunk_data):
            ...
            chunk_end_time = chunk_data.get("metadata", {}).get(
                "end_time", float("inf")
            )
            logger.info(
                f"Processing QA auditor for chunk {chunk_id} (time range: {chunk_start_time}s - {chunk_end_time}s)"
            )
            ...
            for result in re_verify_results:
                ...
                if (
                    detection_end >= chunk_start_time
                    and detection_start <= chunk_end_time
                ):
                    ...
                    if recommendation == "PASS":
                        ...
                        logger.debug(
                            f"Added PASS detection {result.get('detection_id')} at {detection_start}s-{detection_end}s to chunk {chunk_id}"
                        )
                    else:
                        logger.debug(
                            f"Skipped {recommendation} detection {result.get('detection_id')} at {detection_start}s-{detection_end}s for chunk {chunk_id}"
                        )

            logger.info(
                f"Found {len(chunk_detections)} detections for chunk {chunk_id}"
            )
            ...
            chunk_original_text = self._extract_original_chunk_text(
                chunk_data.get("text", ""), chunk_start_time, chunk_end_time
            )

            chunk_masked_text = self._extract_masked_chunk_text(
                masked_transcript, chunk_start_time, chunk_end_time
            )
        ...
        result = {
            ...
            "qa_summary": {
                ...
                "successful": sum(
                    1 for r in qa_results if r.get("status") == "success"
                ),
        ...
        logger.info(
            f"QA auditor processing completed. Processed {len(qa_results)} chunks."
        )
        return result

    def _extract_masked_chunk_text(
        self, masked_transcript: str, start_time: float, end_time: float
    ) -> str:
        """
    ...
    def _extract_original_chunk_text(
