...
class PromptManager:
    def __init__(self, prompt_base_path: Optional[str] = None):
        ...
        self._subagent_configs = [
            ...
            {
                "name": "agent_payment",
                "description": "ทำงานด้าน Payment",
                "file": "agents/agent_payment.md",
            },
    ...
    def consistency_checker(self) -> str:
        ...
        if self._consistency_checker is None:
            self._consistency_checker = self._load_prompt(
                "agents/agent_consistency_checker.md"
            )
    ...
    def consistency_checker_batch(self) -> str:
        ...
        if self._consistency_checker_batch is None:
            self._consistency_checker_batch = self._load_prompt(
                "agents/agent_consistency_checker_batch.md"
            )
    ...
    def compare_chunk_wav_files(self) -> str:
        ...
        if self._compare_chunk_wav_files is None:
            self._compare_chunk_wav_files = self._load_prompt(
                "agents/agent_compare_chunk_wav_files.md"
            )
    ...
    def choose_model_to_transcribe(self) -> str:
        ...
        if self._choose_model_to_transcribe is None:
            self._choose_model_to_transcribe = self._load_prompt(
                "agents/agent_choose_model_to_transcribe.md"
            )
    ...
    def _load_subagents(self) -> Dict[str, Any]:
        ...
        for config in self._subagent_configs:
            try:
                ...
                logger.debug(
                    f"Loaded subagent '{config['name']}' from {config['file']}"
                )

            except Exception as e:
                logger.error(
                    f"Failed to load subagent '{config['name']}' from {config['file']}: {e}"
                )
