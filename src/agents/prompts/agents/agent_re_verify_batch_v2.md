...
### Ex 9: Agent Echo Pattern (Confirmation)
**Input:**
```
[60.0] "สี่สี่สามสอง" (det_01)
[61.5] "สี่สี่สามสองนะคะ"
[62.0] "ห้าห้าหกหนึ่ง" (det_02)
[63.0] "ห้าห้าหกหนึ่งถูกไหมคะ"
```
**Output:**
- **det_01:** PASS (Zone A: No explicit 'บัตร' keyword, but Agent immediately echoes digits (gap < 3s) → Payment confirmation → Card)
- **det_02:** PASS (Zone A: Agent echoes 'ห้าห้าหกหนึ่ง' → Payment confirmation → Card)
...
### Ex 16: Agent Verification (Agent Speaking)
**Input:**
```
[102.0] "ขอรบกวน คุณสมาชิกเเจ้งเลขหน้าบัตรเครดิตเลยค่ะ 5569 ไหมคะ" (det_01)
[103.0] "หนึ่งสองสามสี่ห้า" (det_02)
```
**Output:**
- **det_01:** FAIL (Zone A: Agent is speaking ('ขอรบกวน คุณสมาชิก'), asking for card number. Digits '5569' are in agent's question ('5569 ไหมคะ') → Agent verification, NOT caller's data → Keep)
- **det_02:** PASS (Zone A: Previous line has 'เลขหน้าบัตรเครดิต' → Payment context. Caller responds with 5 digits → Likely card number → Mask)
</examples>
