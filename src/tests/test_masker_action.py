...
def create_mock_transcript():
    ...
    return {
        "transcript": [
            {"text": "สวัสดีครับ ผมชื่อพันธรรมาพิทักษ์พรค่ะ", "start": 0.0, "end": 5.0},
            {
                "text": "หมายอนุญาตเลขที่หกหกศูนย์สองศูนย์ศูนย์สองเก้าสี่สอง",
                "start": 47.92,
                "end": 51.44,
            },
            {"text": "หกหกศูนย์สองศูนย์ศูนย์สองเก้าสี่สอง", "start": 51.44, "end": 64.0},
            {"text": "ห้าสองห้าหกหกแปดสองหนึ่งศูนย์เก้า", "start": 1194.11, "end": 1194.43},
            {"text": "หกแปดสองหนึ่ง", "start": 1198.75, "end": 1199.79},
            {"text": "ศูนย์สองทับสองเก้า", "start": 1211.85, "end": 1213.29},
            {"text": "เบอร์หนึ่งเจ็ดห้าแปด", "start": 593.98, "end": 594.46},
        ]
    }
...
def test_extract_pass_detections():
    ...
    for i, result in enumerate(mock_re_verify_results, 1):
        ...
        print(
            f"  {i}. ID: {result['detection_id']}, Type: {result['detection_type']}, "
    ...
    for i, detection in enumerate(pass_detections, 1):
        print(
            f"  {i}. ID: {detection['id']}, Type: {detection['type']}, "
        ...
        print(
            f"     Time Range: {detection['start_time']:.2f} - {detection['end_time']:.2f}"
        )
...
async def test_masker_action_execute():
    ...
    try:
        ...
        for detection in pass_detections:
            print(
                f"  - ID: {detection['id']}, Type: {detection['type']}, "
