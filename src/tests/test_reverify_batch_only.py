...
class ReVerifyBatchTester:
    ...
    async def test_reverify_batch_on_processed_file(
        ...
        for file_entry in processed_data:
            ...
            credit_card_chunks = [
                chunk
                for chunk in file_data["processed_chunks"]
            ...
            async def process_chunk_batch(i, chunk):
                async with re_verify_semaphore:
                    return await self._process_chunk_with_reverify_batch(
                        chunk, original_transcript, filename
                    )

            chunk_tasks = [
                process_chunk_batch(i, chunk)
                for i, chunk in enumerate(credit_card_chunks)
            ]
            if chunk_tasks:
        ...
        return {"status": "success", "total_files": len(results), "results": results}

    async def _process_chunk_with_reverify_batch(
        self, chunk: Dict[str, Any], original_transcript: Dict[str, Any], filename: str
    ) -> Dict[str, Any]:
        """
        ...
        batch_input = prepare_batch_re_verify_input(
            chunk, detections, original_transcript
        )
        ...
        for detection in detections:
            ...
            result_data = results_map.get(
                det_id, {"status": "error", "error": "Missing from batch result"}
            )
        ...
        return {
            ...
            "summary": self._generate_chunk_summary(
                original_masked_cards, mapped_results
            ),
        }
    ...
    def _generate_chunk_summary(
        ...
        passed = sum(
            1
            for r in batch_reverify_results
        ...
        return {
            ...
            "pass_rate": passed / len(batch_reverify_results)
            if batch_reverify_results
            else 0,
        }
...
async def main():
    ...
    # File paths
    processed_file = (
        "d:/Terrabit/asr_service_server/examples/processed_sample_test_set.json"
    )
    original_files_dir = "d:/Terrabit/asr_service_server/examples/sample_test_set"
    output_file = (
        "d:/Terrabit/asr_service_server/examples/reverify_batch_test_results.json"
    )

    # Run test
    results = await tester.test_reverify_batch_on_processed_file(
        processed_file, original_files_dir, output_file
    )
