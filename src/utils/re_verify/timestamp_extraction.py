...
def extract_detections_with_timestamps(
    ...
    for chunk in processed_chunks:
        logger.debug(
            f"Processing chunk {chunk.get('chunk_id', 'unknown')}, has_credit_card: {chunk.get('has_credit_card', False)}"
        )
        ...
        if chunk.get("has_credit_card") and "masked_credit_cards" in chunk:
            ...
            for detection in chunk["masked_credit_cards"]:
                ...
                logger.debug(
                    f"Added masked credit card detection: {detection.get('type', 'unknown')} at {detection.get('start_time', 0)}"
                )

        # NEW: Extract from Payment Agent results
        if "workflow_result" in chunk:
            logger.debug(
                f"Checking workflow_result in chunk {chunk.get('chunk_id', 'unknown')}"
            )
            if "completed_results" in chunk["workflow_result"]:
                logger.debug(
                    f"Found {len(chunk['workflow_result']['completed_results'])} completed results"
                )
                for result in chunk["workflow_result"]["completed_results"]:
                    if result.get("agent") == "Agent_Payment":
                        ...
                        if (
                            "detections" in payment_result
                            and payment_result["detections"]
                        ):
                            logger.debug(
                                f"Found {len(payment_result['detections'])} Payment Agent detections"
                            )
                            for detection in payment_result["detections"]:
                                if detection.get("pii_type") == "PAYMENT":
                                    logger.debug(
                                        f"Processing PAYMENT detection: {detection.get('raw_value', 'N/A')}"
                                    )
                                    ...
                                    detection_data = {
                                        ...
                                        "context_window": {
                                            "start_time": max(
                                                0,
                                                detection.get("start_time", 0)
                                                - before_seconds,
                                            ),
                                            "end_time": detection.get("end_time", 0)
                                            + after_seconds,
                                            "original_start": detection.get(
                                                "start_time", 0
                                            ),
                                            "original_end": detection.get(
                                                "end_time", 0
                                            ),
                                            "before_seconds": before_seconds,
                                            "after_seconds": after_seconds,
                                    ...
                                    detections.append(detection_data)
                                    logger.debug(
                                        f"Added Payment Agent detection: {detection.get('raw_value', 'N/A')} at {detection.get('start_time', 0)}"
                                    )
                                else:
                                    logger.debug(
                                        f"Skipping non-PAYMENT detection: {detection.get('pii_type', 'unknown')}"
                                    )
                        else:
                            logger.debug("No detections found in Payment Agent result")
                    else:
                        logger.debug(
                            f"Skipping non-Payment Agent result: {result.get('agent', 'unknown')}"
                        )
            else:
    ...
    logger.info(
        f"Extracted {len(detections)} total detections from {len(processed_chunks)} chunks"
    )
    return detections


def extract_detections_by_chunk(
    processed_chunks: List[Dict[str, Any]],
) -> List[Dict[str, Any]]:
    """
    ...
    logger.info(
        f"Extracting detections by chunk from {len(processed_chunks)} processed chunks"
    )
    ...
    for chunk in processed_chunks:
                            ...
                            for detection in payment_result["detections"]:
                                if detection.get("pii_type") == "PAYMENT":
                                    ...
                                    if not is_duplicate:
                                        detection_data = {
                                            "id": detection.get(
                                                "id", f"payment_{len(chunk_detections)}"
                                            ),
                                            "type": "card_number",
                                            "original_text": detection.get(
                                                "raw_value", ""
        ...
        if chunk_detections:
            logger.debug(
                f"Chunk {chunk.get('chunk_id')} has {len(chunk_detections)} detections"
            )
...
def calculate_context_windows(detections: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for detection in detections:
        ...
        detection["context_window"]["duration"] = (
            context["end_time"] - context["start_time"]
        )
        detection["context_window"]["actual_before_seconds"] = (
            context["original_start"] - context["start_time"]
        )
        detection["context_window"]["actual_after_seconds"] = (
            context["end_time"] - context["original_end"]
        )

    return detections


def group_detections_by_proximity(
    detections: List[Dict[str, Any]], proximity_threshold: float = 30.0
    ...
    for detection in sorted_detections[1:]:
        ...
        if (
            ...
            - last_detection["detection"]["end_time"]
            <= proximity_threshold
        ):
