...
def extract_context_with_segments(
    ...
    simple_text = "\n".join(
        [f"[{seg['channel']}]: {seg['text']}" for seg in context_segments]
    )
...
def prepare_re_verify_input(
    ...
    elif "transcript" in original_transcript and isinstance(
        original_transcript["transcript"], dict
    ):
    ...
    if not segments:
        logger.warning(
            f"No segments found in transcript. Available keys: {list(original_transcript.keys())}"
        )
    ...
    logger.debug(
        f"Extracted {len(context_segments)} segments within context window [{context_start:.2f} - {context_end:.2f}]"
    )
...
def prepare_batch_re_verify_input(
    ...
    if not segments:
        logger.warning(
            f"No segments found in transcript. Available keys: {list(transcript_data.keys())}"
        )
            ...
            segments = [
                {
                    "start": 0,
                    "end": context_end,
                    "text": transcript_data["simple_text"],
                    "channel": "unknown",
                }
            ]
            logger.debug("Created fallback segment from simple_text")
...
def analyze_context_for_false_positive(
    context_text: str, detection_type: str = "card_number"
) -> Dict[str, Any]:
    """
